    ...
    public static class UpdateService extends Service<Void>{
        ...
        protected UpdateService(TableView<Car> tableView){
            setOnSucceeded(workerStateEvent -> {
                ...
                if (!GarageMenuController.hasResults && !GarageMenuController.needClear){
                    try {
                        tableView.getItems().setAll(DataStorageService.cars());
                    } catch (IOException e) {
                        e.printStackTrace();
                ...
                else if(!newCarController.isReady && !newCarController.wasClosed){
                    try {
                        tableView.getItems().setAll(DataStorageService.cars());
                    } catch (IOException e) {
                        e.printStackTrace();
                ...
                protected Void call() throws Exception {
                    ...
                    if(onNewCar) {
                        while (!newCarController.isReady && !newCarController.wasClosed) {
                            Thread.sleep(10);
                            System.out.println("onNewCar");
                            System.out.println("waiting");
                        }
                        return null;
                    }
                    else if (onMenu){
                        while (!GarageMenuController.isReady && !GarageMenuController.wasClosed) {
                            Thread.sleep(10);
                        }
                        return null;
                    }
